...
class Admin extends AdminModule
{
    ...
    // Tambahan: Update Tanggal Pulang SEP 2.0 (PUT)
    public function postUpdtglplg()
    {
        date_default_timezone_set('UTC');
        $no_sep = $_POST['no_sep'] ?? '';
        $status_pulang = $_POST['status_pulang'] ?? '';
        ...
        $no_lp_manual = $_POST['no_lp_manual'] ?? '';
        $user = $_POST['user'] ?? '';
        $this->initBpjs();
        $tStamp = strval(time() - strtotime("1970-01-01 00:00:00"));
        $key = $this->consid . $this->secretkey . $tStamp;
    ...
    // Tambahan: List Data Update Tanggal Pulang SEP (GET)
    public function anyListupdtglplg()
    {
        date_default_timezone_set('UTC');
        $bulan = $_GET['bulan'] ?? '';
        $tahun = $_GET['tahun'] ?? '';
    ...
    // Helper untuk inisialisasi credential BPJS
    private function initBpjs()
    {
