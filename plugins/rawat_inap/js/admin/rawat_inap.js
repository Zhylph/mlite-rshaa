...
// Handler Pulangkan Pasien (Update Tanggal Pulang SEP)
$("#form").on("click", "#btn_pulangkan_pasien", function() {
  ...
  $("#inputNoLPManual").val('');
  $("#divMeninggal").hide();
  $("#divTglMeninggal").hide();
  $("#notifPulangkanPasien").html('');
  $("#modalPulangkanPasien").modal("show");
});

// Show/hide field meninggal berdasarkan status pulang
$("#modalPulangkanPasien").on("change", "#inputStatusPulang", function() {
  var status = $(this).val();
  if(status == '4') {
    $("#divMeninggal").show();
    $("#divTglMeninggal").show();
    $("#inputNoSuratMeninggal").prop('required', true);
    $("#inputTglMeninggal").prop('required', true);
  } else {
    $("#divMeninggal").hide();
    $("#divTglMeninggal").hide();
    $("#inputNoSuratMeninggal").prop('required', false);
    $("#inputTglMeninggal").prop('required', false);
  }
});

$("#modalPulangkanPasien").on("click", "#submitPulangkanPasien", function() {
  var no_sep = $("#inputNoSep").val();
  var status_pulang = $("#inputStatusPulang").val();
  var no_surat_meninggal = $("#inputNoSuratMeninggal").val();
  var tgl_meninggal = $("#inputTglMeninggal").val();
  var tgl_pulang = $("#inputTglPulang").val();
  var no_lp_manual = $("#inputNoLPManual").val();
  var user = $("#inputUser").val();
  var baseURL = mlite.url + '/' + mlite.admin;
  $("#notifPulangkanPasien").html('<span class="text-info">Memproses...</span>');
  $.ajax({
    url: baseURL + '/rawat_inap/updtglplg?t=' + mlite.token,
    method: 'POST',
    data: { 
      no_sep: no_sep, 
      ...
      no_lp_manual: no_lp_manual,
      user: user
    },
    success: function(res) {
      var data = {};
      try { data = JSON.parse(res); } catch(e) { data = res; }
      if(data && data.metaData && data.metaData.code == '200') {
        $("#notifPulangkanPasien").html('<span class="text-success">' + data.metaData.message + '</span>');
      } else {
        $("#notifPulangkanPasien").html('<span class="text-danger">' + (data.metaData ? data.metaData.message : res) + '</span>');
      }
    },
    error: function(xhr) {
      $("#notifPulangkanPasien").html('<span class="text-danger">Terjadi kesalahan koneksi.</span>');
    }
  });
});

// Handler List Data Update Tanggal Pulang SEP
$("#btnListUpdtglplg").on("click", function() {
  $("#notifListUpdtglplg").html('');
  $("#formListUpdtglplg")[0].reset();
  $("#tableListUpdtglplg tbody").html('');
  $("#modalListUpdtglplg").modal("show");
});

$("#modalListUpdtglplg").on("click", "#submitListUpdtglplg", function() {
  var bulan = $("#bulanListUpdtglplg").val();
  var tahun = $("#tahunListUpdtglplg").val();
  var filter = $("#filterListUpdtglplg").val();
  var baseURL = mlite.url + '/' + mlite.admin;
  $("#notifListUpdtglplg").html('<span class="text-info">Memproses...</span>');
  $.ajax({
    url: baseURL + '/rawat_inap/listupdtglplg?t=' + mlite.token,
    method: 'GET',
    data: { bulan: bulan, tahun: tahun, filter: filter },
    success: function(res) {
      var data = {};
      try { data = JSON.parse(res); } catch(e) { data = res; }
      if(data && data.metaData && data.metaData.code == '200' && data.response && data.response.list) {
        var rows = data.response.list;
        var html = '';
        for(var i=0; i<rows.length; i++) {
          var r = rows[i];
          html += '<tr>' +
            '<td>' + (r.noSep||'') + '</td>' +
            '<td>' + (r.noSepUpdating||'') + '</td>' +
        ...
        $("#tableListUpdtglplg tbody").html(html);
        $("#notifListUpdtglplg").html('<span class="text-success">Data ditemukan: ' + rows.length + '</span>');
      } else {
        $("#notifListUpdtglplg").html('<span class="text-danger">' + (data.metaData ? data.metaData.message : res) + '</span>');
        $("#tableListUpdtglplg tbody").html('');
      }
    },
    error: function(xhr) {
